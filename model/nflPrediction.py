...
from s.nfl.boxscore import Boxscores, Boxscore
...
def get_schedule(year):
    ...
    for w in range(len(weeks)):
        ...
        for g in range(len(week_scores.games[date_string])):
            game = pd.DataFrame(
                week_scores.games[date_string][g], 
                index = [0]
            )
            [[
                'away_name', 
    ...
    try:
        ...
        try:
            if game_df.loc[0,'away_score'] > game_df.loc[0,'home_score']:
                ...
                away_team_df = pd.merge(away_team_df, pd.DataFrame({
                    'game_won' : [1], 
                    'game_lost' : [0]
                }), left_index = True, right_index = True)

                home_team_df = pd.merge(home_team_df, pd.DataFrame({
                    'game_won' : [0], 
                    'game_lost' : [1]
                }), left_index = True, right_index = True)
        ...
        away_stats_df = game_stats.dataframe[[
            'away_first_downs', 
        ...
        home_stats_df = game_stats.dataframe[[
            'home_first_downs', 
...
def game_data_up_to_week(weeks, year):
    ...
    for w in range(len(weeks)):
        ...
        for g in range(len(week_scores.games[date_string])):
            ...
            away_team_df, home_team_df = game_data(
                game_df,
                game_stats
            )
...
def agg_weekly_data(schedule_df, weeks_games_df, current_week, weeks):
    ...
    for w in range(1,len(weeks)):
        ...
        agg_weekly_df = weeks_games_df[weeks_games_df.week < weeks[w]].drop(columns=[
            'score',
            'week',
            'game_won', 
            'game_lost'
        ...
        win_loss_df = win_loss_df.drop(columns = [
            'game_won', 
            'game_lost'
        ])
        ...
        agg_weekly_df = agg_weekly_df.drop(columns = [
        ...
        agg_weekly_df = pd.merge(
            away_df,
            home_df,
            left_on = ['away_name', 'away_abbr', 'home_name', 'home_abbr', 'winning_name', 'winning_abbr', 'week'], 
            right_on =['away_name', 'away_abbr', 'home_name', 'home_abbr', 'winning_name', 'winning_abbr', 'week']
        )
...
def get_elo():
    ...
    elo_df['team1'] = elo_df['team1'].replace([
        'KC', 'JAX', 'CAR', 'BAL', 'BUF', 'MIN', 'DET', 'ATL', 'NE','WSH', 'CIN', 'NO', 'SF', 'LAR', 'NYG', 'DEN', 
        'CLE', 'IND', 'TEN', 'NYJ', 'TB', 'MIA', 'PIT', 'PHI', 'GB', 'CHI', 'DAL', 'ARI', 'LAC', 'HOU', 'SEA', 'OAK'
    ], [
        'kan','jax','car', 'rav', 'buf', 'min', 'det', 'atl', 'nwe', 'was', 'cin', 'nor', 'sfo', 'ram', 'nyg', 'den', 
        'cle', 'clt', 'oti', 'nyj','tam','mia', 'pit', 'phi', 'gnb', 'chi', 'dal', 'crd', 'sdg', 'htx', 'sea', 'rai' 
    ])

    elo_df['team2'] = elo_df['team2'].replace([
        'KC', 'JAX', 'CAR', 'BAL', 'BUF', 'MIN', 'DET', 'ATL', 'NE','WSH','CIN', 'NO', 'SF', 'LAR', 'NYG', 'DEN', 'CLE', 
        'IND', 'TEN', 'NYJ', 'TB', 'MIA', 'PIT', 'PHI', 'GB', 'CHI', 'DAL', 'ARI', 'LAC', 'HOU', 'SEA', 'OAK'
    ], [
        'kan','jax','car', 'rav', 'buf', 'min', 'det', 'atl', 'nwe', 'was','cin', 'nor', 'sfo', 'ram', 'nyg', 'den', 'cle', 
        'clt', 'oti', 'nyj', 'tam','mia', 'pit', 'phi', 'gnb', 'chi', 'dal', 'crd', 'sdg', 'htx', 'sea', 'rai' 
    ])
...
def merge_rankings(agg_games_df,elo_df):
    ...
    agg_games_df = agg_games_df.drop(columns = [
        'elo1_pre', 
        'elo2_pre', 
        'qb1_value_pre',
        'qb2_value_pre'
...
# To predict the probabilities for NFL Game Outcomes, It can use our logistic regression model Example:
